...
```bash
# 預設：本地 LLM server（http://localhost:1234，模型 qwen3.6-35b-a3b）
...
```bash
# 第一條：load_model
(crontab -l 2>/dev/null; echo "45 1 * * * cd /Users/guangzhenglee/Workspace/agent && /Library/Frameworks/Python.framework/Versions/3.10/bin/python3 load_model.py >> /tmp/load_model.log 2>&1") | crontab -

# 第二條：daily-brief
(crontab -l 2>/dev/null; echo "0 2 * * * cd /Users/guangzhenglee/Workspace/agent && /Library/Frameworks/Python.framework/Versions/3.10/bin/python3 main.py \"/daily-brief\" >> /tmp/daily_brief.log 2>&1") | crontab -
```
